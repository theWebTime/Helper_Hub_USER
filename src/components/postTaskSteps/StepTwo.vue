  ...
  <div>
    ...
    <div v-for="type in selectedSubservice.types" :key="type.slug" class="pt-6">
      ...
      <div class="flex flex-wrap items-center justify-start font-medium gap-3">
        <button v-for="detail in type.details" :key="detail.id" @click="formData[type.slug] = detail.id" :class="[
    ...
    <!-- ✅ Gender (only for Monthly Service i.e. service_id === 2) -->
    <div class="pt-6" v-if="selectedSubservice?.service_id === 2">
      <p class="pb-4 font-medium text-n300">Gender?*</p>
      <div class="flex gap-4">
        <button @click="formData.gender = false" :class="[
          'px-6 py-2 rounded-md border',
          formData.gender === false
            ? 'bg-n900 text-white border-n900'
            : 'bg-n30 hover:bg-n900 hover:text-white'
        ]">Male</button>
        <button @click="formData.gender = true" :class="[
          'px-6 py-2 rounded-md border',
    ...
    <div class="pt-6">
      ...
      <textarea v-model="formData.special_instruction" class="w-full p-3 border rounded-md resize-none" rows="4"
        placeholder="Mention any specific requests, notes, or location hints"></textarea>
    </div>
  </div>
</template>
...
const formData = ref<Record<string, any>>({
  ...
  gender: false,                     // ✅ default value (Male/Female only for monthly)
