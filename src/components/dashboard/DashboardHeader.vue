    ...
    <div class="4xl:large-container max-4xl:container">
      <div class="text-s1 flex items-center justify-between py-6">
        <div class="flex items-center justify-start gap-3 pb-1">
          <button @click="showMobileMenu = true" class="mobileMenuOpenButton text-3xl lg:hidden">
            <PhList />
          </button>
          <router-link to="/dashboard/home">
            <img :src="logo" alt="Helper Hub logo"
        ...
        <nav class="max-lg:hidden">
          <ul class="flex items-center justify-center gap-2 font-medium xxl:gap-6">
            <li v-for="{ id, name, link } in dashboardMenu" class="" :key="id">
              <router-link :to="link" :class="{ 'text-b300': route.path === link }"
    ...
    <nav class="">
      ...
      <div :class="{
        ...
        <div class="fixed top-0 flex w-full items-center justify-between bg-r300 p-4 sm:p-8">
          ...
          <button @click="showMobileMenu = false" class="cursor-pointer !text-3xl">
            <PhX />
          </button>
        </div>
